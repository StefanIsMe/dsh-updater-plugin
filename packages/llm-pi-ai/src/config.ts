...
import { DEFAULT_MAX_REQUEST_FILE_TEXT_CHARS } from './file-request.ts'
...
export interface PiAiProviderProfile {
  ...
  /**
   * Total extracted-text characters generic-file attachments may contribute to
   * one provider request (the paperclip non-image path). When a request's
   * attachments exceed it, later files degrade to name-only notes until the
   * request fits, so a document-heavy session keeps completing requests.
   */
...
export interface ResolvedPiAiProviderProfile
  ...
  requestImageMaxBytes: number
  /** Positive per-request extracted attachment-text budget after defaulting. */
  maxRequestFileTextChars: number
...
const profile = z.object({
  ...
  maxRequestFileTextChars: z.number().step(1).min(1).default(DEFAULT_MAX_REQUEST_FILE_TEXT_CHARS),
...
export function resolveProfiles(
  ...
  for (const [provider, source] of entries) {
    ...
    const maxRequestFileTextChars = source.maxRequestFileTextChars ?? DEFAULT_MAX_REQUEST_FILE_TEXT_CHARS
    if (!Number.isSafeInteger(maxRequestFileTextChars) || maxRequestFileTextChars <= 0) {
      throw new Error(`llm-pi-ai: provider "${provider}" maxRequestFileTextChars must be a positive safe integer`)
    }

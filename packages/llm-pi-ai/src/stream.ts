...
export function mapStopReason(message: AssistantMessage, contextWindow?: number): FinishReason {
  ...
  switch (message.stopReason) {
    ...
    case 'pending':
    case 'deferred':
      // 0.84 non-terminal markers must never reach the terminal mapper;
      // treating one as a completed response would silently drop the turn.
      return {
        kind: 'error',
        failure: { message: `pi-ai produced a non-terminal "${message.stopReason}" stop reason`, code: 'PI_AI_ERROR' },
      }
  }
}

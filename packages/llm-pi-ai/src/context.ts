...
import { collectFileRefs, DEFAULT_MAX_REQUEST_FILE_TEXT_CHARS, fileBlockText, fileNoteText, prepareRequestFiles } from './file-request.ts'
import type { PreparedFileText } from './file-request.ts'
...
// The name-only generic-file projection (fileNoteText) lives beside its
// contents-bearing successor (fileBlockText) in ./file-request.ts.
...
async function userContent(
  ...
  requestFiles?: ReadonlyMap<AttachmentId, PreparedFileText>,
  ...
  for (const block of blocks) {
    switch (block.type) {
      ...
      case 'file': {
        // Generic files ride provider requests as extracted text: a docx or
        // decodable-text attachment is flattened into a delimited block, and
        // anything that cannot be represented (binary payload, failed read,
        // spent budget) degrades to the name-only note so the model still
        // knows the attachment exists and can ask for it another way.
        const prepared = requestFiles?.get(block.attachment.attachmentId)
        content.push({ type: 'text', text: prepared === undefined ? fileNoteText(block.attachment) : fileBlockText(prepared) })
        break
      }
      case 'tool-result':
        {
...
function textOnlyContext(options: GenerateOptions, onReplayDegrade?: (reason: string) => void): PiContext {
  ...
  for (const message of options.messages) {
    ...
    // Generic-file references stay model-visible even without the attachment
    // service: the note needs no byte resolution, only the durable metadata.
    const notes = message.content
      .filter((block): block is Extract<ContentBlock, { type: 'file' }> => block.type === 'file')
      .map(block => fileNoteText(block.attachment))
      .join('\n')
    const combined = notes === '' ? text : text === '' ? notes : `${text}\n${notes}`
    const results = message.content.filter(block => block.type === 'tool-result')
    if (combined.length > 0 || results.length === 0) messages.push({ role: 'user', content: combined, timestamp: 0 })
 ...
 * @param maxRequestFileTextChars - request-level budget on extracted attachment text; omission uses {@link DEFAULT_MAX_REQUEST_FILE_TEXT_CHARS}.
...
export function toPiContext(
  ...
  maxRequestFileTextChars?: number,
): Promise<PiContext>
export function toPiContext(
  options: GenerateOptions,
  attachments?: AttachmentStore,
  ...
  return attachments === undefined
    ...
    : toPiContextWithImages(options, attachments, onReplayDegrade, maxRequestImageBytes, requestImagePolicy, maxRequestFileTextChars)
}
...
async function toPiContextWithImages(
  ...
  maxRequestFileTextChars: number = DEFAULT_MAX_REQUEST_FILE_TEXT_CHARS,
): Promise<PiContext> {
  ...
  // Generic-file attachments ride this request as extracted text under a
  // shared character budget (see ./file-request.ts); every referenced file is
  // projected exactly once per unique attachment id.
  const fileRefs = collectFileRefs(exactMessages)
  const requestFiles = await prepareRequestFiles(
    [...fileRefs.values()],
    attachments,
    maxRequestFileTextChars,
    options.signal,
  )

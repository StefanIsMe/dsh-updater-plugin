...
import { contentHasFile } from './file-request.ts'
...
export class PiAiAdapter extends LlmAdapter {
  ...
  private async * streamWithSnapshot(
    ...
    try {
      ...
      // Generic files resolve through the same durable service: without it the
      // request still completes, projecting name-only notes instead of contents.
      const containsFile = contentHasFile(options.messages)
      ...
      const context = attachments === undefined
        ...
        }, profile.maxRequestFileTextChars)
      ...
      try {
        while (true) {
          ...
          // pi-ai 0.84 relabels caller-abort outcomes as generic errors (the
          // signal reason becomes the message), so the caller-signal check that
          // governs the throw path below must govern terminal chunks too: an
          // aborted caller means the turn's outcome IS an abort.
          if (value.type === 'finish' && options.signal?.aborted && value.reason.kind === 'error') {
            yield { type: 'finish', reason: { kind: 'aborted', failure: { message: 'pi-ai request aborted by caller', code: 'ABORTED' } } }
            continue
          }
          yield value
        }
